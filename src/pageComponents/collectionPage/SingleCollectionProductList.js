...
const SingleCollectionProductList = ({
				...
				<div className='flex items-center gap-4 mb-4 lg:mx-0 mx-2'>
					...
					<p
						className={`cursor-pointer px-2 lg:px-4 flex items-center h-7 lg:h-10 rounded-10	 text-sm lg:text-base 
      						${activeTab === "products" ? "border border-purple-500 bg-purple-100 text-purple-700 font-semibold" : "text-gray-700"}  `} onClick={() => setActiveTab("products")}>
						PRODUCTS
					</p>
    ...
    `}
			...
			{((activeTab === "products" || !isSingleCollectionSharedPage) && blogCollectionPage.collection_name && (
				<>
														...
														{Object.entries(filteredFirstValue).map(
															([key, values]) => {
																if (key === "custom_filter") {
																	return savedCustomFilter.map((tag, index) => (
																		<SwiperSlide
																			key={`${key}-${index}`}
																			style={{ width: "auto" }}>
												...
												{user_name === super_admin ? (
													<img
														src={
															...
															getFinalImageUrl(blogCollectionPage.profile_image)
														}
									...
									View all
										...
										<div>
											{blogCollectionPage.description ? (
												<p className='text-sm lg:text-lg text-justify whitespace-pre-line leading-normal ellipsis_2'>
													{blogCollectionPage.description}
												</p>
											) : null}
										</div>
			...
			{(activeTab === "faq" && singleCollection?.faqs?.length > 0) && isSingleCollectionSharedPage && (
